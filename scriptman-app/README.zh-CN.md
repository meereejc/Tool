...
- 当前脚本工作区支持：
  ...
  - 运行 / 停止
  - 实时日志流
- `Install suggestions` 现在会把环境配置说明和命令一起放进同一个代码块里，
  `Copy command` 复制的也是整段代码块内容，而不只是最后一行命令
...
- `@sm:dep` 现在既可以表示命令行依赖，也可以表示 Python 包依赖
- 命令行依赖可以直接写：
  - `@sm:dep ffmpeg`
  - `@sm:dep cmd:ffmpeg`
- Python 包依赖可以写成以下任一种形式：
  - `@sm:dep opencv-python`
  - `@sm:dep py:opencv-python`
  - `@sm:dep pip:opencv-python`
- 对于 Python 脚本，ScriptMan 会用脚本实际运行时对应的解释器检查 Python
  包是否安装，并生成类似下面的建议命令：
...
- 如果包装在另一个虚拟环境、Conda 环境，或另一个 Python 解释器里，而不是
  ScriptMan 当前为该脚本解析到的那个运行时，应用仍然会正确提示该依赖缺失
